...
int main(void){
  double Mstar_init = MS_UM(1.);
  double Fstar_init = FS_Guo(1.);
  double Mstar_pred = Ms_UM(1.);
  double Fstar_pred = Fs_Guo(1.);
  printf("Mstar_init, Fstar_init, Mstar_pred, Fstar_pred = %f %f %f %f \n", 
		  Mstar_init, Fstar_init, Mstar_pred, Fstar_pred);
}
